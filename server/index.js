        ...
        if(!cadet){
         ...
         const isMatch = await bcrypt.compare(password,cadet.password);
             ...
             if(!isMatch){   // If the passwords don't match, send an unauthorized response and exit.
               ...
               const token= jwt.sign(//JWT consists of three parts, separated by dots (.) once they are converted       //E.G. ---- Formula:HMACSHA256(base64UrlEncode(header) + "." + base64UrlEncode(payload), secret) The final JWT is the concatenation of these three encoded parts, separated by periods: [Header].[Payload].[Signature]
